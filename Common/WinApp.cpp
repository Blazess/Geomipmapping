...
bool WinApp::InitWindow()
{
	...
	g_pDInput->Init(m_hWnd,m_hInstance,DISCL_FOREGROUND | DISCL_NONEXCLUSIVE,DISCL_FOREGROUND | DISCL_NONEXCLUSIVE);
...
void WinApp::CalculateFPS(int M,int N,int MAX_LEVEL)
{
	...
	if(m_timer.TotalTime() - begin >= 1.f)
	{
		...
		text<<L"      Resolution: "<<(pow(2,MAX_LEVEL-1)+1)*N<<L"*"<<(pow(2,MAX_LEVEL-1)+1)*M;
